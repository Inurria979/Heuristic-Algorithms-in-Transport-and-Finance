...
Code for the lab activity (see `docs/image.png`): write a 3-page tutorial article on the basic
...
| Path | Contents |
|---|---|
| `src/markowitz_osqp.ipynb` | **Main deliverable**: load data → solve with OSQP → times table and frontier plots |
| `data/` | OR-Library instances `port1..5.txt` and reference frontiers `portef1..5.txt` |
| `docs/Portfolio Optimization Problem.tex`, `docs/Javier S, Jesus I, Portfolio Optimization Problem.pdf` | The tutorial article (source and PDF) |
| `docs/esquema_articulo.md`, `docs/contexto_paper.md` | Outline and working context of the article (Spanish) |
| `docs/Paper a Mano.pdf` + transcription | Handwritten draft |
| `docs/Beasley 2014 - ….pdf`, `docs/Heuristics.pdf` | Reference papers |
| `docs/01a Optimization Modeling 101 2026.pptx.pdf` | Course slides |
| `docs/image.png` | Assignment statement |
| `results/` | Per-instance frontier CSVs, `summary.csv/.md`, `environment.json`, `figures/` (the notebook writes `figures/frontiers.{pdf,png}` here) |
| `ignore/` (git-ignored) | Original OR-Library download and zips, archived extended script `markowitz_osqp.py`, old `main.py` |
...
Open `src/markowitz_osqp.ipynb` in VS Code/Jupyter with the `.venv` kernel and run all cells
